...
class BaseHMM(Base):
    ...
    def gen_one_data(self, step):
        ...
        """
        #初始化第一个隐藏态
        state = [np.min(np.argwhere(np.cumsum(self.Pi) > np.random.random()))]
        obs = [np.min(np.argwhere(np.cumsum(self.B[state[-1]]) > np.random.random()))]
        #生成第一个观测序列
        for _ in range(1, step):
            state.append(np.min(np.argwhere(np.cumsum(self.A[state[-1]]) > np.random.random())))
            obs.append(np.min(np.argwhere(np.cumsum(self.B[state[-1]]) > np.random.random())))
        return state, obs
    ...
    def _alpha(self, obs, t=None):
        ...
        for i in range(_t + 1):
            alpha = (self.Pi if i == 0 else alpha @ self.A) * self.B[:, obs[i]]
        return alpha
    ...
    def _beta(self, obs, t=None):
        ...
        for i in range(len(obs)-t): # 注意求beta(t)(i) 使用的是 obs(t+1)
            beta = np.ones(shape=self.n_q, dtype=np.float) if i == 0 else\
                np.sum(self.A * self.B[:, obs[len(obs) - i]] * beta, axis=1)
        return beta
    ...
    def backward_prob(self, obs):
        ...
        """
        _obs = obs if isinstance(obs, np.ndarray) else np.array(obs)
        return np.sum(self.Pi * self._beta(obs) * self.B[:, obs[0]])
    ...
    def _gamma(self, obs, t):
        ...
        :return:
        """
        prob = self._alpha(obs, t) * self._beta(obs, t)
    ...
    def fb_prob(self, obs, t=None):
        ...
        return np.sum(self._alpha(obs, t) * self._beta(obs, t))
    ...
    def _xi(self, obs, t): #ksi 求转移概率
        ...
        :return:`
         """
        alpha_t = np.mat(self._alpha(obs, t))
        beta_t = self._beta(obs, t + 1)
        obs_beta = np.mat(self.B[:, obs[t + 1]] * beta_t)
        alpha_obs_beta = np.asarray(alpha_t.T * obs_beta)
    ...
    def fit(self, obs_data, max_step=1000, init=True):
        # 利用Baum-Welch算法学习
        #初始化需要学习的参数
        self.A = np.ones((self.n_q, self.n_q)) / self.n_q if init or self.A is None else self.A
        self.B = np.ones((self.n_q, self.n_v)) / self.n_v if init or self.B is None else self.B
        ...
        while step < max_step: #循环次数
            ...
            # 更新 A
            self.A = xi / gamma.reshape((-1, 1))
            # 更新 B
            self.B = gamma_b / (gamma + gamma_end).reshape(-1, 1)
            # 更新 Pi
            self.Pi = Pi / len(obs_data)
            step += 1
